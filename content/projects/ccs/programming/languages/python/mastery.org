...
#+PROPERTY: LOG_INTO_DRAWER t      ; put all logs into :LOGBOOK:
#+OPTIONS: d:nil p:nil             ; do not export drawers or property drawers
;; (optional) keep TODO/DONE in headings (default). If you wanted to hide them:
;; #+OPTIONS: todo:nil
...
(defun ab/wrap-logs-into-logbook ()
  "Wrap standalone CLOSED/State/CLOCK lines under a heading into a :LOGBOOK: drawer."
  (interactive)
  (require 'org-element)
  (save-excursion
    (goto-char (point-min))
    (while (re-search-forward
            "^\\(CLOSED:.*\\|[ \t]*- State .*\\|[ \t]*CLOCK:.*\\)$" nil t)
      (let* ((ctx (org-element-context))
             (in-drawer (org-element-lineage ctx '(drawer property-drawer) t)))
        (unless in-drawer
          (beginning-of-line)
          (let ((start (point)))
            ;; grab the contiguous block of log lines
            (while (looking-at
                    "^\\(CLOSED:.*\\|[ \t]*- State .*\\|[ \t]*CLOCK:.*\\)$")
              (forward-line 1))
            (let ((end (point)))
              (goto-char start) (insert ":LOGBOOK:\n")
              (goto-char end)   (insert ":END:\n"))))))))
