...
% Explicit preview block (more reliable than \PreviewEnvironment in some builds)
\usepackage[active,tightpage]{preview}
\pagestyle{empty}

% algorithm2e niceties (optional)
\DontPrintSemicolon
\setlength{\algomargin}{1em}
\let\vec\mathbf

\begin{document}
\begin{preview}                 % <— explicit preview open
\begin{tikzpicture}[x=1pt,y=1pt,baseline=(ALG.north)]
  % Put the algorithm in a minipage; add a top strut so the header isn't clipped
  \node[anchor=north west, inner sep=0pt, outer sep=0pt] (ALG) at (0,0) {%
    \begin{minipage}{520pt}\vspace*{1ex}
      \begin{algorithm}[H]
\SetAlgoLined
...
\caption{BST-Search$(T,k)$}
  ...
  % Force the page bbox to exactly this rectangle by emitting a real (invisible) path:
  \path[draw,opacity=0,line width=0pt]
    (ALG.north west) rectangle (ALG.south east);

\end{tikzpicture}
\end{preview}                 % <— explicit preview close
\end{document}
