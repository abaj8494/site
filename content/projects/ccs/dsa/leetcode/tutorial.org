...
permutations and combinations both refer to selecting items from some set of data:
permutation refers to the ordering or arranging the data in a different type of way
combinations is referring to the different selections of data from that dataset.
...
we use shallow copies in backtracking because they efficiently capture the
current state of the path when dealing with simple immutable elements like ints
could have used deep copy, but that would make this solution less efficient.
deep copies are only necessary when dealing with nested or mutable data
