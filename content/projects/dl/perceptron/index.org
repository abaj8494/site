...
Now of course a purely geometric solution can be constructed for this, and we can spin one up in a moment to match our arithmetic to, but first let us update the weights manually and see how many iterations of this algorithm are required to find such a solution.
...
1. The Perceptron Algorithm does not always converge to the optimal solution.
   - I already knew it would not find a solution to a non-linear problem, that makes sense of course considering that you are fitting a linear decision boundary to the data.
   - But in fact, the reason why it does not converge in this case is because *we chose bad initialisation weights*!
2. My code above differs from the dot product code you saw in the first figure;
   - I calculate \(s\), a useful auxillary value and then do comparisons on that variable.
   - This was the code I wrote during my semester to solve the above problem. The only difference was that I was given different initialisation weights as such:
...
#+RESULTS:
[[file:./.ob-jupyter/3dcd1212605cd9a31aad8f52803555396db5c7e0.png]]
...
** A (slightly) harder example; still 2D
...
*** Method Extraction:
At this point, we keep reusing the same code and so let us refactor the rogue code into a more disciplined class:
...
there is a bug in your non-class code. this class code manages to converge on the 0 vector. you must investigate!
